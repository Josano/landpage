       ...
       $(this).on('keyup', function(){
           ...
           if (cep.length == 8){
                ...
                $.getJSON( "https://webmaniabr.com/api/1/cep/"+cep+"/?app_key="+correios_app_key+"&app_secret="+correios_app_secret, function( data ) {
                    ...
                    if(data.endereco){ $(endereco).prop("readonly", true); }else{ $(endereco).prop("readonly", false); }
                    if(data.bairro){ $(bairro).prop("readonly", true); }else{ $(bairro).prop("readonly", false); }
                    if(data.cidade){ $(cidade).prop("readonly", true); }else{ $(cidade).prop("readonly", false); }
                    if(data.uf){ $('#uf option:not(:selected)').prop('disabled', true); }else{ $('#uf option:not(:selected)').prop('disabled', false); }
                    if (cep.substr(5,3) == '000'){ $( endereco ).focus(); }else{$( numero ).focus(); }
                });
